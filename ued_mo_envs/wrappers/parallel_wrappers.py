...
class ParallelAdversarialVecEnv(SubprocVecEnv):
    ...
    def get_param_dims(self):
        self._assert_not_closed()
        self.remotes[0].send(('level_params_vec', None))
        self.waiting = True
        level_params_vec = self.remotes[0].recv()
        self.waiting = False
        return len(level_params_vec[0])

    def get_param_info(self):
        self._assert_not_closed()
        self.remotes[0].send(('param_info', None))
        self.waiting = True
        param_info = self.remotes[0].recv()
        self.waiting = False
        return param_info[0]
    ...
    def __getattr__(self, name):
        ...
        elif name == 'param_dims':
            return self.get_param_dims()
