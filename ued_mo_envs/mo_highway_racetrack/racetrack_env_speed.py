...
class RacetrackEnvSpeed(AbstractEnv):
    """
    Variation of the original continuous control racetrack environment 
    with speed rewards and customizable road layout.
    ...
    In this variation, the agent is rewarded for speed in addition to the original rewards.
    See original: https://github.com/Farama-Foundation/HighwayEnv/blob/7415379bb741d993557c88d50577df0de190959d/highway_env/envs/racetrack_env.py
    """
        ...
        config.update(
            {
                ...
                "action": { # action are ordered as [throttle, steering] with range [[-1, 1], [-1, 1]]
                    "type": "ContinuousAction",
                    "longitudinal": True, # enable throttle control in this environment variation (for controlling speed)
                    "lateral": True,
                },
                ...
                "reward_speed_range": [20, 30], # vehicle is rewarded for speed in this range
    ...
    def _reward(self, action: np.ndarray) -> float:
        ...
        reward = utils.lmap( # normalize reward
            reward, 
            [
                self.config["collision_reward"],
                self.config["lane_centering_reward"] + self.config["high_speed_reward"]
            ], 
            [0, 1]
        )
    ...
    def _rewards(self, action: np.ndarray) -> Dict[Text, float]:
        ...
        return {
            ...
            "action_reward": np.linalg.norm(action), # Penalize large actions => 
                                                     # raw maximum is sqrt(2) ~ 1.4, scaled by -0.3 => maximum is -0.42
