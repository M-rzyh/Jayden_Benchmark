...
class FruitTreeEnv(gym.Env, EzPickle):
    ...
    def reset_random(self):
        ...
        """
        # Generate random weights for the leaf nodes
        num_leaves = int(2 ** self.tree_depth)
        random_fruits = np.random.uniform(low=0.0, high=self.max_reward, size=(num_leaves, self.reward_dim))

        # Update the tree with new random fruits
        branches = np.zeros((int(2 ** self.tree_depth - 1), self.reward_dim))
