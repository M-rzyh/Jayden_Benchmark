...
class MOSACDiscreteSharedCNN(MOPolicy):
    """Multi-objective Soft Actor-Critic (SAC) algorithm for discrete action spaces.
    ...
    def __init__(
        ...
        self.q_feature_extractor = CustomCNN(self.obs_shape, features_dim=net_arch[0]).to(self.device)
        ...
        self.q_optimizer = optim.Adam(list(self.q_feature_extractor.parameters()) + list(self.qf1.parameters()) + list(self.qf2.parameters()), lr=self.q_lr, eps=1e-4)
    ...
    def __deepcopy__(self, memo):
        ...
        copied.q_feature_extractor = deepcopy(self.q_feature_extractor)
        ...
        copied.q_optimizer = optim.Adam(list(copied.q_feature_extractor.parameters()) + list(copied.qf1.parameters()) + list(copied.qf2.parameters()), lr=self.q_lr, eps=1e-4)
        if self.autotune:
    ...
    def get_save_dict(self, save_replay_buffer: bool = False) -> dict:
        ...
        save_dict = {
            ...
            'q_feature_extractor_state_dict': self.q_feature_extractor.state_dict(),
    ...
    def load(self, save_dict: Optional[dict] = None, path: Optional[str] = None, load_replay_buffer: bool = True):
        ...
        self.q_feature_extractor.load_state_dict(save_dict['q_feature_extractor_state_dict'])
    ...
    def update(self):
        ...
        with th.no_grad():
            ...
            processed_next_obs = self.q_feature_extractor(mb_next_obs)
        ...
        with autocast():
            processed_obs = self.q_feature_extractor(mb_obs)
