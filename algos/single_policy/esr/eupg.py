...
class EUPG(MOPolicy, MOAgent):
    ...
    def __init__(
        ...
        if log and parent_rng is None:
            self.setup_wandb(self.project_name, self.experiment_name, wandb_entity, wandb_group, wandb_tags)
    ...
    def train(
        ...
        for _ in range(1, total_timesteps + 1):
            ...
            with th.no_grad():
                ...
                obs_tensor = th.tensor(obs).to(self.device)
                action = self.__choose_action(obs_tensor.unsqueeze(0), accrued_reward_tensor)
