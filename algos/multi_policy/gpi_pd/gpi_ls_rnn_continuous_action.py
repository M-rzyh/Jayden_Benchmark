...
from mo_utils.networks import layer_init, mlp, polyak_update, get_grad_norm
...
class FeaturesExtractor(nn.Module):
    def __init__(self, obs_dim, hidden_dim, rnn_layers=1, recurrent_type='lstm'):
        super().__init__()
        self.obs_dim = obs_dim

        self.state_features = mlp(obs_dim, -1, [hidden_dim])
        self.shortcut_state_features = mlp(obs_dim, -1, [hidden_dim])
...
class GPILSRNNContinuousAction(MOAgent, MOPolicy):
        ...
        **kwargs
        ...
        if self.use_gpi:
            if num_envs > 1: # evaluating using vectorised envs
                ...
                stacked_weight_support = self.stacked_weight_support
