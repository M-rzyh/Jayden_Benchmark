...
class Envelope(MOPolicy, MOAgent):
    ...
    def __init__(
        ...
        Args:
            ...
            per_alpha: The alpha parameter for PER.
    ...
    def update(self):
        ...
        for g in range(self.gradient_updates):
            ...
            critic_loss = huber(td_error.abs(), self.min_priority)

            if self.homotopy_lambda > 0:
                wQ = th.einsum("br,br->b", q_value, w)
                wTQ = th.einsum("br,br->b", target_q, w)
                auxiliary_loss = huber((wQ - wTQ).abs(), self.min_priority)
            ...
            if self.per:
                ...
                priority = priority.clip(min=self.min_priority) ** self.per_alpha
