"""Modified wrappers from MO-Gymanasium due to compatibility issues with Gymnasium v1.0.0a2."""
...
from gymnasium.wrappers.vector.common import RecordEpisodeStatistics as VectorRecordEpisodeStatistics
...
class MORecordEpisodeStatistics(RecordEpisodeStatistics, gym.utils.RecordConstructorArgs):
    ...
    def __init__(self, env: gym.Env, gamma: float = 1.0, deque_size: int = 100, stats_key: str = "episode"):
        """This wrapper will keep track of cumulative rewards and episode lengths.

        Args:
            env (Env): The environment to apply the wrapper
            gamma (float): Discounting factor
            deque_size: The size of the buffers :attr:`return_queue`, :attr:`length_queue` and :attr:`time_queue`
            stats_key: The info key for the episode statistics
        """
        gym.utils.RecordConstructorArgs.__init__(self, gamma=gamma, buffer_length=deque_size)
        RecordEpisodeStatistics.__init__(self, env, buffer_length=deque_size)
    ...
    def step(self, action):
        ...
        obs, vec_reward, terminated, truncated, info = self.env.step(action)
        self.episode_returns += vec_reward
        self.episode_lengths += 1

        # CHANGE: The discounted returns are also computed here
        self.disc_episode_returns += vec_reward * np.repeat(self.gamma**self.episode_lengths, self.reward_dim).reshape(
        ...
        if terminated or truncated:
            ...
            self.disc_episode_returns = np.zeros(self.reward_dim, dtype=np.float32)
        
        return obs, vec_reward, terminated, truncated, info

class MOVectorRecordEpisodeStatistics(VectorRecordEpisodeStatistics, gym.utils.RecordConstructorArgs):
    """This wrapper will keep track of cumulative rewards and episode lengths.
    ...
    def __init__(self, env: gym.Env, gamma: float = 1.0, deque_size: int = 100):
        ...
        """
        gym.utils.RecordConstructorArgs.__init__(self, gamma=gamma, buffer_length=deque_size)
        VectorRecordEpisodeStatistics.__init__(self, env, buffer_length=deque_size)
        # CHANGE: Here we just override the standard implementation to extend to MO
        # We also take care of the case where the env is vectorized
        self.reward_dim = self.env.unwrapped.reward_space.shape[0]
    ...
    def step(self, action):
        ...
        self.episode_returns[self.prev_dones] = np.zeros(self.reward_dim, dtype=np.float32)
        ...
        # CHANGE: The discounted returns are also computed here
        self.disc_episode_returns[~self.prev_dones] += rewards[~self.prev_dones] * np.repeat(self.gamma**self.episode_lengths, self.reward_dim).reshape(
            self.episode_returns[~self.prev_dones].shape
        )
