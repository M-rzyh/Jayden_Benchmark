...
class RecurrentPrioritizedReplayBuffer:
    """Recurrent Prioritized Experience Replay Buffer."""
    ...
    def __init__(
        ...
        """Initialize the Recurrent Prioritized Replay Buffer.
        ...
        Args:
            ...
            min_priority: Minimum priority of the buffer
        """
        self.max_size = max_size // sequence_length
        self.sequence_length = sequence_length
        self.ptr = 0
        self.size = 0

        # Buffers to store sequences of experiences
        self.obs = np.zeros((max_size, sequence_length, ) + (obs_shape), dtype=obs_dtype)
    ...
    def add(self, obs, action, reward, next_obs, done, priority=None):
        """Add a new sequence of experiences to the buffer.

        Args:
            obs: Sequence of observations
            action: Sequence of actions
            reward: Sequence of rewards
            next_obs: Sequence of next observations
            done: Sequence of done flags
            priority: Priority of the new experience sequence
        """
        self.obs[self.ptr] = np.array(obs).copy()
        self.next_obs[self.ptr] = np.array(next_obs).copy()
        self.actions[self.ptr] = np.array(action).copy()
    ...
    def sample(self, batch_size, to_tensor=False, device=None):
        ...
        Args:
            ...
            to_tensor:  Whether to convert the batch to tensors
            device: Device to move the tensors to

        Returns:
            batch: Batch of experience sequences
        """
        idxes = self.tree.sample(batch_size)
    ...
    def update_priorities(self, idxes, priorities):
        ...
        Args:
            idxes: Indexes of the sequences to update
            priorities: New priorities of the sequences
        """
        self.min_priority = max(self.min_priority, priorities.max())
        self.tree.batch_set(idxes, priorities)
